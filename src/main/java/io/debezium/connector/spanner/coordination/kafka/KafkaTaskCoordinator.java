...
/**
 * Default, Kafka-backed implementation of the partition-coordination SPI. Wraps the connector's
 * existing Kafka components (sync topic publisher/listener, rebalancing-topic leader election,
 * consumer-group membership) so the wiring in {@code SynchronizationTaskContext} depends only on the
 * SPI interfaces. Behaviour is identical to the pre-refactor connector.
 */
