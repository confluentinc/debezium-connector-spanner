...
public class TakePartitionForStreamingOperation implements Operation {
    ...
    private TaskSyncContext takePartitionForStreaming(TaskSyncContext taskSyncContext) {
        ...
        try {
            ...
            List<Partition> resolvedPartitions = partitionFactory.getPartitions(toStreaming);
            Map<String, Partition> partitionMap = new HashMap<>();
            for (int i = 0; i < toStreaming.size(); i++) {
                partitionMap.put(toStreaming.get(i).getToken(), resolvedPartitions.get(i));
            }
